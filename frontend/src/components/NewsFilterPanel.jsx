    ...
    >
      ...
      <div style={{ borderTop: '1px solid #E2E8F0', paddingTop: '16px' }}>
        ...
        <div style={{ fontSize: '12px', color: '#64748B', marginBottom: '8px' }}>
          Describe what you're looking for — AI will intelligently filter the news for you.
        </div>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
          <textarea
            placeholder={`e.g. "Show me infrastructure news that's affecting citizens negatively in ${constituency || 'this area'}"`}
            value={aiQuery}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <button
              ...
              disabled={aiFilterLoading || !aiQuery.trim()}
              style={{
                padding: '8px 16px',
                border: 'none',
                borderRadius: '7px',
                background: aiFilterLoading || !aiQuery.trim() ? '#E2E8F0' : '#2563EB',
                color: aiFilterLoading || !aiQuery.trim() ? '#64748B' : '#FFFFFF',
                fontSize: '13px',
                fontWeight: 600,
                cursor: aiFilterLoading || !aiQuery.trim() ? 'not-allowed' : 'pointer',
                fontFamily: 'inherit',
                whiteSpace: 'nowrap',
              }}
            >
              {aiFilterLoading ? 'Filtering...' : 'Apply AI Filter'}
            </button>
            {aiFilterActive && (
              <button
                onClick={handleClear}
